        ...
        if st.button("Save Recording"):
            try:
                ...
                saved_path = data_storage.save_audio_recording(
                    audio_bytes=audio_bytes,  # Pass the bytes, not the UploadedFile object
            ...
            for file in sorted(os.listdir(recordings_dir)):
                if file.endswith(".wav"):
                    ...
                    with col1:
                        st.write(f"📄 {file}")
                    with col2:
                        st.audio(os.path.join(recordings_dir, file))

    # Then in your recording section:
    if st.button("Save and Analyze Recording"):
        try:
            # Save recording
            audio_bytes = audio_data.read()
            saved_path = data_storage.save_audio_recording(
            ...
            st.write("Saved path:", saved_path)
            ...
            for sentiment, score in results['scores'].items():
                st.progress(score, text=f"{sentiment}: {score:.2f}")
